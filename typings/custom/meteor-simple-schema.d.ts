...
// meteor package: aldeed:simple-schema
...
interface SimpleSchema<T> {
  ...
  label(propertyName: string): string;
	...
	/**
	 * The clean method takes the object to be cleaned as its first argument
   * and the following optional options as its second argument.
   ...
   * and the following optional options as its second argument.
	 ...
	 */
  clean(
    objectOrModifier: Mongo.Modifier,
    options?: MeteorSimpleSchema.CleaningOptions): Mongo.Modifier;
  ...
  newContext(): MeteorSimpleSchema.ValidationContext<T>;
	 ...
	 * That way, the context is automatically persisted by name,
   * allowing you to easily rely on its reactive methods.
	 */
  namedContext(name: string): MeteorSimpleSchema.ValidationContext<T>;

	/**
	 * Adds a custom validation function that is called for all keys
   * for a specific SimpleSchema instance.
	 * All information for the validation is provided in the function's context.
	 * Cast this to SimpleSchema.CustomValidationThis in order to make use of that context
	 */
  addValidator(customValidator: Function): void;
...
declare module MeteorSimpleSchema {
  ...
  interface PropertyDefinition {
    ...
    | ObjectConstructor | DateConstructor | SimpleSchema<any> | ArrayConstructor
    | [StringConstructor] | [NumberConstructor] | [BooleanConstructor]
    | [ObjectConstructor] | [DateConstructor] | [SimpleSchema<any>];
		 ...
		 *
		 * If you require a field that changes its meaning in some circumstances
     * you can provide a callback function as a label.
		 */
    label?: string | (() => string);
		 ...
		 * - For items within an array, or when the key name ends with ".$",
     *   the optional option has no effect.
		 * 	 That is, something cannot be "required" to be in an array.
		 * - If a key is required at a deeper level,
     *   the key must have a value only if the object it belongs to is present.
    ...
    max?: number;
		...
		/**
		 * Set to true to indicate that the numeric value, set by min,
     * is to be treated as an exclusive limit.
		 * Set to false (default) to treat limit as inclusive.
		 */
    exclusiveMin?: boolean;

		/**
		 * Set to true to indicate that the numeric value, set by max,
     * is to be treated as an exclusive limit.
		 * Set to false (default) to treat limit as inclusive.
		 */
    inclusiveMin?: boolean;
		...
		/**
		 * Set to true if type is Number or [Number] and you want to allow non-integers.
    ...
    regEx?: RegExp | RegExp[];
		 ...
		 *
		 * Custom object types are treated as blackbox objects by default.
     * However, when using collection2,
